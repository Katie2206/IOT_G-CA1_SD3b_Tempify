...
@app.route("/")
def index():
    # logged_in_message = f"Hello {session['name']}! <br/> <a href='/login'><button>Logout</button></a>"
...
def protected_area():
    ...
    return render_template("protected_area.html", logged_in=("google_id" in session), Username=session.get("name"), user_id=session['google_id'], online_users=my_db.view_all_users(), admin_id=config.get('GOOGLE_ADMIN_ID'))
